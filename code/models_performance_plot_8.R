...
for(i in loop_list){
  ...
  legend("bottomright", 
         legend = c(
           paste("GEMINI-ICU (AUC = ", roc_need[roc_need$model_name == "pred_f_num_text",]$roc_auc_use, ")", sep = ""),           
...
for(i in loop_list){
  ...
  plot_decision_curve( list(model1, model2, model3, model4, model5),
                       curve.names = c("GEMINI-ICU", "Structured data score", "preICU risk score", "SAPSII", "SOFA"),
                       col = c("blue", "red", "green", "yellow", "purple"),
...
for(i in loop_list){
  ...
  for(j in list("pred_f_num", "preICU_risk_score", "sofa", "saps", "eldericu", "xgb", "rf")){
    print(paste("--------- GEMINI-ICU vs.", j, " ----------",sep = ""))
